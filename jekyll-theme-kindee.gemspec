# frozen_string_literal: true

Gem::Specification.new do |spec|
  spec.name = "jekyll-theme-kindee"
  spec.version = "0.1.0"
  spec.authors = ["hatakawas"]
  spec.email = ["hatakawas@163.com"]

  spec.summary = "Jekyll-theme-kindee is a fresh theme for jekyll blog implemented with flex layout."
  spec.homepage = "https://github.com/hatakawas/jekyll-theme-kindee.git"
  spec.license = "MIT"

  spec.files = `git ls-files -z`.split("\x0").select { |f| f.match(%r!^(assets|_layouts|_includes|_sass|_data|LICENSE|README)!i) }

  spec.add_runtime_dependency "jekyll", "~> 4.0"
  spec.add_runtime_dependency "jekyll-feed", "~> 0.13"
  spec.add_runtime_dependency "jekyll-sitemap", "~> 1.4"
  spec.add_runtime_dependency "jekyll-seo-tag", "~> 2.6"
  spec.add_runtime_dependency "jekyll-data", "~> 1.1"

  spec.add_development_dependency "bundler", "~> 2.0"
  spec.add_development_dependency "rake", "~> 13.0"
end
